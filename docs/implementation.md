...
- Đã dọn riêng container/network/volume QA của Compose project `prn232lab1_se193274`; chỉ chứa seed và dữ liệu kiểm thử của lượt này. Kết quả 10/10 là của công cụ được cung cấp, không phải cam kết điểm chính thức của giảng viên.
- Kiểm tra cuối phát hiện LocalDB process riêng không còn sống, /health8088 báo DB unavailable. Không sửa hệ thống/không xóa DB cũ: dừng đúng API process25120, chạy lại8088 bằng image Docker đã build --no-cache từ ZIP, Compose project `prn232-lab1`. Docker volume mới có dữ liệu seed, không phải dữ liệu chuyển từ LocalDB; file LocalDB cũ vẫn giữ. Dùng Docker cho lần chạy tiếp theo theo README.
